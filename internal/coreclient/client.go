...
// orginal gorpc BATCH code removed
...
func clientHandleConnection(c *Client, conn io.ReadWriteCloser) {
	...
	pendingRequests := make(map[string]*AsyncResult)
...
func clientWriter(c *Client, w io.Writer, pendingRequests map[string]*AsyncResult, pendingRequestsLock *sync.Mutex, stopChan <-chan struct{}, done chan<- error) {
	var err error
	defer func() { done <- err }()
...
func clientReader(c *Client, r io.Reader, pendingRequests map[string]*AsyncResult, pendingRequestsLock *sync.Mutex, done chan<- error) {
	var err error
	defer func() {
		if r := recover(); r != nil {
			if err == nil {
